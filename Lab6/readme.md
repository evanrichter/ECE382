...
Another point of frustration was motor noise due to the motion of the wheels. I saw on the oscilloscope noise interfering with the IR sensor, on the data line. It was strong enough to pull the input pin high or low at random. There was a consistent 1.5kHz pulse to 0 then back to 1 when the motors were moving backwards slowly. Using a simple resistor and capacitor as a high pass filter didn't give good enough performance in cutting out the noise. I tried many different capacitor configurations and sizes, with no consistent results until I put even larger capacitors on the 12v rail.

### Conclusion
The strongest lessons learned were in debugging throughout this lab. Also, taking a look at the tech docs before trying to implement my own dual PWM functionality would have saved a lot of work. The overall approach of my code is to have the control scheme implemented in main, with interrupts handling the low-level grinding, and helper methods to make code easier to read. This approach should be helpful in later labs.
